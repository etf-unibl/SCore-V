...
![SCore-V Architecture](docs/score_v_docs/score_v_architecture.drawio.png)
...
The following waveform illustrates the execution of the instruction:

> `lw x2, 0(x0)`

![Waveform example](docs/score_v_docs/score_v_msim.drawio.png)
*Figure: ModelSim waveform showing execution of the `lw x2, 0(x0)` instruction.*


This instruction appears in instruction_memory.txt at program counter value PC = 32 and has the following binary encoding:
...
This command compiles the VHDL source files and runs all available testbenches.
...
> `py run.py testbench_lib.score_v_tb.test_score_v`

**Running Simulation with Waveform Visualization**

To open the simulation in ModelSim with waveform visualization enabled:

>`py run.py testbench_lib.score_v_tb.test_score_v --gui`

After the ModelSim GUI opens, waveform signals can be added using (tlc):

>`add wave *`

>`run -all`

This allows observation of internal processor signals and instruction execution behavior.
